...
# ─────────────────────────────────────────────────────────────────────────────
# ОТКРЫТАЯ ССЫЛКА-ПРИГЛАШЕНИЕ (без привязки к email)
# ─────────────────────────────────────────────────────────────────────────────


@router.post(
    "/companies/{company_id}/team/invite-link",
    response_model=OpenInviteLinkResponse,
    status_code=status.HTTP_201_CREATED,
    summary="Создать открытую ссылку-приглашение",
    description="Создаёт ссылку без привязки к email — любой кто перейдёт по ней получит указанную роль.",
)
async def create_open_invite_link(
    company_id:   UUID,
    body:         OpenInviteLinkRequest,
    current_user: CurrentUser = Depends(CanManageTeam),
    db:           AsyncSession = Depends(get_db),
) -> OpenInviteLinkResponse:
    _INVITE_EXPIRE_DAYS = 7
    ...
    # Открытая ссылка хранится как инвайт с email = OPEN_LINK_{token[:8]}
    placeholder_email = f"open_link_{token[:8]}@axiom.invite"
    invitation = CompanyInvitation(
        id=uuid.uuid4(),
        company_id=company_id,
        invited_by_user_id=current_user.user_id,
...
async def accept_invite(
    ...
    # Определяем email: для open-link берём из body, иначе из приглашения
    is_open_link = inv.email.startswith("open_link_") and "@axiom.invite" in inv.email
    if is_open_link:
        # Открытая ссылка — email и пароль обязательны
        if not body.email:
            raise HTTPException(
                status.HTTP_422_UNPROCESSABLE_ENTITY,
                "Для открытой ссылки-приглашения необходимо передать `email`.",
            )
        effective_email = body.email
    else:
        effective_email = inv.email

    # Ищем или создаём пользователя
    user_result = await db.execute(select(User).where(User.email == effective_email))
    ...
    if user is None:
        ...
        user = User(
            ...
            full_name=body.full_name or effective_email.split("@")[0],
            is_active=True,
